...
void task11_1() {
    ...
    while (getline(inputFile, inputLine)) {
        ...
        int wordCount = 0;
        bool Validator;
        cout << "String input: " << inputLine << endl;
        while (stream >> word) {
            Validator = true;
            if (wordCount <= 1)
                for (char c : word)
                    if (!isalpha(c)) {
                        cout << "Non valid word " << word << endl;
                        Validator = false;
                        break;
                    }
...
void task14_1() {
    ...
    if (maxWordIndex != -1)
        outputFile << "Word with the most 'a' is placed in " << maxALine << " line " << ++maxWordIndex << " position." << endl;
    else
        outputFile << "Word with the most a doesnt exist" << endl;
