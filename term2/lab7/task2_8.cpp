...
void oneByOne(ListManager& To, const ListManager& from1, const ListManager& from2) {
    To.clear();
    Address* p1 = from1.head;
    Address* p2 = from2.head;

    while (p1 || p2) {
        if (p1) {
            insert(To, new Address(p1->name, p1->city));
            p1 = p1->next;
        }
        if (p2) {
            insert(To, new Address(p2->name, p2->city));
            p2 = p2->next;
        }
    }
}
...
int main() {
    while (true) {
        try {
            ...
            case 9:
                oneByOne(lists[RESULT], lists[LIST1], lists[LIST2]);
